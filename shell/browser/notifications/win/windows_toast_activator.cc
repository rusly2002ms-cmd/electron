...
void EnsureClsidRegistry() {
  ...
  std::wstring key_path = L"Software\\Classes\\CLSID\\" + std::wstring(clsid);
        ...
        if (SUCCEEDED(existing.As(&store))) {
          ...
          if (SUCCEEDED(store->GetValue(PKEY_AppUserModel_ID, &pv_id)) &&
              SUCCEEDED(store->GetValue(PKEY_AppUserModel_ToastActivatorCLSID,
                                        &pv_clsid))) {
            if (pv_id.vt == VT_LPWSTR && pv_clsid.vt == VT_CLSID &&
                pv_id.pwszVal && pv_clsid.puuid) {
              static const CLSID kActivatorClsidStruct = {
                  0x9e0a8b8f,
                  0x6f2b,
                  0x4c63,
                  {0xb5, 0xf0, 0x9b, 0x1f, 0x3f, 0x8e, 0x4d, 0x1a}};
...
void HandleToastActivation(const std::wstring& invoked_args,
  ...
  if (type == L"action" && target->delegate()) {
    ...
    // Look for any selection input (key starts with L"selection") – we encode
    // ids as the numeric index of the selected option.
    for (const auto& entry : inputs) {
      std::wstring_view key_view(entry.key);
      if (base::StartsWith(key_view, L"selection",
                           base::CompareCase::SENSITIVE)) {
        // Value is the id attribute of <selection>; attempt to parse int
        // safely.
        int parsed = -1;
